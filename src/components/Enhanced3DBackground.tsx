...
function ConnectingLines() {
  ...
  const points2 = [new THREE.Vector3(6, 6, -10), new THREE.Vector3(-6, -4, -12)];
  const points3 = [new THREE.Vector3(0, 8, -15), new THREE.Vector3(-4, -8, -18)];

  return (
    <group ref={linesRef}>
      <Line points={points1} color="#ff6b35" transparent opacity={0.3} lineWidth={2} />
      <Line points={points2} color="#4fc3f7" transparent opacity={0.4} lineWidth={2} />
      <Line points={points3} color="#ffa726" transparent opacity={0.2} lineWidth={2} />
    </group>
  );
}
...
function DataStreams() {
    ...
    if (streamRef.current) {
      streamRef.current.children.forEach((child, index) => {
        ...
        if (child instanceof THREE.Mesh && child.material instanceof THREE.Material) {
          (child.material as any).opacity = Math.sin(state.clock.elapsedTime + index) * 0.3 + 0.1;
        }
      });
    }
  });
  ...
  for (let i = 0; i < 8; i++) {
    streams.push(
      <mesh key={i} position={[Math.random() * 16 - 8, 0, -15 + Math.random() * 5]}>
...
function EnergyOrbs() {
  ...
  return (
    <group ref={orbsRef}>
      <Float speed={2} rotationIntensity={1} floatIntensity={2} position={[-10, 4, -8]}>
        <mesh>
      ...
      <Float speed={1.5} rotationIntensity={1.5} floatIntensity={1.5} position={[10, -4, -12]}>
        <mesh>
      ...
      <Float speed={1.8} rotationIntensity={0.8} floatIntensity={2.5} position={[0, 8, -18]}>
        <mesh>
